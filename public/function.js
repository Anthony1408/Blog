...
var isSearch = 0;
function hide_main() {
    document.getElementById('content_pagination').style.display = 'none';
}
function show_main() {
    document.getElementById('content_pagination').style.display = 'block';
}
...
var changePage = function (num) {
    ...
    if (isSearch == 0) {
        ...
        xhr.open("GET", "/search_left_content" + "/page/" + num, true);
    }
    xhr.send();
    xhr.onload = function () {
        var output = this.responseText;
        $("#content_pagination").html(output);
    };
};

var search = function (param) {
    var cb = document.getElementById('cb');
    var firstDate = document.getElementById("dateText1").value
    var secandDate = document.getElementById("dateText2").value
    var text = document.getElementById("search_text").value
    var xhr = new XMLHttpRequest();
    if (cb.checked) {
        xhr.open("GET", '/search' + "/FisrtDate/" + firstDate + "/SecondDate/" + secandDate +"/Param/"+ param, true);
    } else {
        xhr.open("GET", '/search/' + text + "/Param/" + param, true);
    }
    xhr.send();
    xhr.onload = function () {
        var output = this.responseText;
...
var addPublication = function () {
    var author = document.getElementById("author").value
    var img = document.getElementById("img").value
    var content = document.getElementById("content").value
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/addPublication" + "/Author/" + author + "/Img/" + img + "/Content/" + content, true);
    xhr.send();
}
...
function calendar(param) {
    ...
    if (param == 1) {
        cal = new fcp.Calendar(document.getElementById("cal_placeholder1"));
        document.getElementById('cal_placeholder1').style.display = 'block';
    } else {
        cal = new fcp.Calendar(document.getElementById("cal_placeholder2"));
        document.getElementById('cal_placeholder2').style.display = 'block';
    }
    cal.onselect = function (date) {
        var d = date.getDate() + '.' + date.getMonth() + '.' + date.getFullYear();
        if (param == 1) {
            document.getElementById('dateText1').value = d;
            document.getElementById('cal_placeholder1').style.display = 'none';
        } else {
            document.getElementById('dateText2').value = d;
            document.getElementById('cal_placeholder2').style.display = 'none';
        }
    }
}
...
function showOrHide() {
    ...
    cat = document.getElementById('DateFirst');
    cat1 = document.getElementById('DateSecond');
    if (cb.checked) {
        searchForm
        cat.style.display = "block";
        cat1.style.dispalay = "block";
    }
    else {
